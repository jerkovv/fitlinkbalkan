-- request_membership_purchase je od pocetka pokusavao da obavesti trenera,
-- ali insert u notifications je koristio nepostojecu kolonu "user_id"
-- (tabela ima recipient_id), pa je EXCEPTION WHEN OTHERS THEN NULL tiho
-- gutao gresku pri svakom pozivu. Ispravljeno na tacan oblik koji
-- ostatak projekta koristi (recipient_id + recipient_role + sender_id +
-- athlete_id), plus dodato postovanje trenerovih podesavanja obavestenja
-- (kategorija 'payments'), isti obrazac kao notify_booking_created.
