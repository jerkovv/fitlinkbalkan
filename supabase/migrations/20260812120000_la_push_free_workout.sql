-- Live Activity push za SLOBODAN trening (day_id IS NULL).
--
-- Problem: app pokrene aktivnost lokalno sa isFreeWorkout=true (ispravan raspored:
-- stoperica + puls + kalorije), ali cim stigne prvi puls sa sata ovaj triger posalje
-- push koji PREPISE ceo ContentState - bez isFreeWorkout, sa praznim exerciseName i
-- setNumber/totalSets iz reda. Nativni widget tada padne na planiran raspored i
-- ispise besmisleno "Serija 1/0".
--
-- Fix: za slobodnu sesiju salji isFreeWorkout=true + activeCalories, bez serija.
-- Planiran trening ostaje bajt-identican dosadasnjem ponasanju.
...
BEGIN
    ...
    IF v_key IS NOT NULL THEN
      ...
      IF v_trainer_id IS NOT NULL THEN
        ...
        IF v_tr_active IS TRUE AND v_tr_token IS NOT NULL THEN
          ...
          -- struktura odmah; puls/kalorije throttle 30s
          IF v_tr_struct
             OR v_tr_last IS NULL
             OR (now() - v_tr_last) >= interval '30 seconds' THEN
  ...
  -- Slobodan trening = sesija bez plana (day_id NULL).
  SELECT (l.day_id IS NULL) INTO v_is_free
  FROM public.workout_session_logs l
  WHERE l.id = NEW.session_log_id;

  IF COALESCE(v_is_free, false) THEN
    -- Nema vezbe ni serija: nativni widget na osnovu isFreeWorkout prelazi na
    -- svoj raspored (stoperica + puls + kalorije).
    v_content := jsonb_build_object(
      'exerciseName',    'Slobodan trening',
      'setNumber',       0,
      'totalSets',       0,
      'hrZone',          v_hr_zone,
      'isResting',       false,
    ...
    IF NEW.current_active_calories IS NOT NULL THEN
      v_content := v_content || jsonb_build_object(
        'activeCalories', round(NEW.current_active_calories)::int
      );
    END IF;
  ELSE
