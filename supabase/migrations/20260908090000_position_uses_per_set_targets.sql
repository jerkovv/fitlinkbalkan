-- Cilj tekuce serije, a ne jedan broj za celu vezbu.
--
-- watch_compute_position je vracala planned_reps/planned_weight iskljucivo iz
-- roditeljskog reda (assigned_program_exercises), iako per-set ciljeve trener
-- upisuje u assigned_program_exercise_sets - i iako je pauza vec citana odatle.
-- Posledica: kad se serija zavrsi bez izricite kilaze (sat bez kesiranog plana,
-- motor na telefonu, replay iz reda akcija), _engine_complete_set je u set_logs
-- upisao staru vrednost sa vezbe umesto onoga sto je trener zadao za bas tu
-- seriju. Trener zada 55 kg za drugu seriju, a u istoriji osvane 40.
--
-- Sada se cilj cita isto kao pauza: per-set red ako postoji, inace roditelj.
-- Ponavljanja mogu biti raspon ("8-12"), pa se uzima vodeci broj - isto pravilo
-- koje sat vec primenjuje na svom prikazu.
...
BEGIN
  ...
  SELECT aps.rest_seconds, aps.reps, aps.weight_kg
    INTO v_per_set_rest, v_per_set_reps, v_per_set_weight
  ...
  -- Cilj BAS ove serije. Raspon ("8-12") daje vodeci broj; prazan tekst se ne
  -- racuna kao zadata vrednost, pa se pada na vezbu.
  v_reps := COALESCE(
    NULLIF(substring(COALESCE(v_per_set_reps, '') from '^[0-9]+'), '')::int,
    v_row.planned_reps);
  v_weight := COALESCE(v_per_set_weight, v_row.planned_weight);
