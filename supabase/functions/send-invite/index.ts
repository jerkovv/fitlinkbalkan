...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: trProfile } = await admin
      .from("profiles")
      .select("full_name")
      .eq("id", trainerId)
      .single();
    let trainerName: string | null = trProfile?.full_name ?? null;
    if (!trainerName) {
      const { data: trRow } = await admin
        ...
        .eq("id", trainerId)
        .single();
      trainerName = trRow?.studio_name ?? null;
    }

    // 2) Šalji invite mejl
    const PUBLIC_SITE_URL = "https://app.fitlink.rs";
