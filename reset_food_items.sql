-- =====================================================================
-- FitLink — KORAK 1: Reset food_items tabele po novoj šemi
-- Pokreni OVO u Supabase SQL Editor PRE CSV uvoza.
-- =====================================================================

-- 1) Obriši sve plan/log redove koji referenciraju food_items
--    (food_id postaje "nepostojeći" nakon truncate-a)
DELETE FROM public.nutrition_plan_meal_items WHERE food_id IS NOT NULL;

-- Ako postoji food_logs tabela (može da ne postoji još), očisti je
DO $$
BEGIN
...
-- 3) Dodaj nedostajuće kolone (postoje već: name, category, kcal_per_100g, P/C/F)
...
-- 4) NOT NULL na osnovne kolone (CSV ih sve ima)
ALTER TABLE public.food_items
...
-- 5) UNIQUE na name (da budući uvozi mogu ON CONFLICT)
DO $$
BEGIN
...
-- 6) Indeksi za pretragu i filter
...
-- 7) RLS — svi mogu da čitaju, samo authenticated da ubace
...
-- 8) Trigger za updated_at (ako već nemaš generic funkciju)
